...
namespace MultiShark
{
    internal static class Program
    {
        ...
        private static void Main(string[] pArgs)
        {
            ...
            {
                if (MessageBox.Show(null, "Did you install WinPcap first? If you did, then try to run MultiShark in Administrator Mode, else press 'No' to go to the install page of WinPcap.", "Interface Error", MessageBoxButtons.YesNo, MessageBoxIcon.Error) == System.Windows.Forms.DialogResult.No)
                {
            ...
            AppDomain.CurrentDomain.UnhandledException += (sender, args) =>
            {
                ...
                System.IO.File.WriteAllText("MultiShark Error.txt", e.ToString());

                if (MessageBox.Show("Exception occurred. Open error in notepad?", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    System.Diagnostics.Process.Start("notepad", "\"MultiShark Error.txt\"");
                }
            };
