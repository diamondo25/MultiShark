...
namespace MultiShark
{
    partial class MainForm : Form
    {
        ...
        {
            InitializeComponent();
            Text = "MultiShark " + Program.AssemblyVersion;
        ...
        private void mDockPanel_ActiveDocumentChanged(object pSender, EventArgs pArgs)
        {
            if (!mClosed)
            {
                var session = mDockPanel.ActiveDocument as SessionForm;
        ...
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (sessions > 5)
            {
                doSaveQuestioning = MessageBox.Show("You've got " + sessions + " sessions open. Say 'Yes' if you want to get a question for each session, 'No' if you want to quit MultiShark.", "MultiShark", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes;
            }

            while (doSaveQuestioning && sessionForms.Count > 0)
            {
                var ses = sessionForms[0];
                if (!ses.Saved)
                {
                    ses.Focus();
                    DialogResult result = MessageBox.Show(string.Format("Do you want to save the session '{0}'?", ses.Text), "MultiShark", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning);
